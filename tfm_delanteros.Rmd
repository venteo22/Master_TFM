...
```{r}
defensas<-df_players[df_players$position=='sa',]
...
legend("topleft", c("defensas", "Centrocampista",'Delanteros'), border="black", fill = c("blue", "gold1",'red'))
```
...
text(paste("Correlación:", round(cor(defensas$normvalue, defensas$outliers_score), 2)), x = 25, y = 95)
```
```{r}
# Creamos el gráfico
...
# Correlación de Pearson
text(paste("Correlación:", round(cor(centrocampistas$normvalue, centrocampistas$outliers_score), 2)), x = 25, y = 95)
```
...
# Correlación de Pearson
text(paste("Correlación:", round(cor(delanteros$normvalue, delanteros$outliers_score), 2)), x = 25, y = 95)
```
...
sorare_sa=merge(x=defensas[,c('Player','value','normvalue','age','club','Pos')],y=sorare,by='Player')
...
```{r}
library(caret)
ss <- preProcess(as.data.frame(sorare_sa$Mediana), method=c("range"))
 
mediana_def <- predict(ss, as.data.frame(sorare_sa$Mediana))
colnames(mediana_def)[1]<-'normmediana'
sorare_sa<-cbind(sorare_sa,mediana_def)
sorare_sa<-sorare_sa %>% relocate(normmediana, .after = value)
```
...
```{r}
# Creamos el gráfico
plot(sorare_sa$normvalue, sorare_sa$normmediana, pch = 19, col = "lightblue")

# Línea de regresión
abline(lm(sorare_sa$normmediana ~ sorare_sa$normvalue), col = "red", lwd = 3)

# Correlación de Pearson
text(paste("Correlación:", round(cor(sorare_sa$normvalue, sorare_sa$normmediana), 2)), x = 25, y = 95)
```
```{r}
# Creamos el gráfico
...
# Correlación de Pearson
text(paste("Correlación:", round(cor(sorare_centrocampista$normvalue, sorare_centrocampista$normmediana), 2)), x = 25, y = 95)
```

```{r}
# Creamos el gráfico
plot(sorare_delantero$normvalue, sorare_delantero$normmediana, pch = 19, col = "lightblue")

# Línea de regresión
abline(lm(sorare_delantero$normmediana ~ sorare_delantero$normvalue), col = "red", lwd = 3)

# Correlación de Pearson
text(paste("Correlación:", round(cor(sorare_delantero$normvalue, sorare_delantero$normmediana), 2)), x = 25, y = 95)
```
```{r}
paste0('Correlación de Pearson defensas: ',cor(sorare_sa$normvalue, sorare_sa$normmediana))
...
abs_diff_defensas <- abs(defensas$normvalue - sorare_sa$normvalue)
...
Construimos tres modelos lineales según la categoría del jugador. Primero empezamos con los jugadores de la sa. 
...
Padefensass (Pases Asistentes): Significativa con un valor p < 0.05, sugiriendo una relación positiva.

Pas3rd (Pases en el tercio de campo): Significativa con un valor p < 0.05, sugiriendo una relación positiva.

TouDefPen (Toques en el área siva del penalti): Muy significativa con un valor p < 0.01, mostrando una fuerte relación positiva.
...
# Imprimir la mediana de MAE y MAPE
...
plot(residuos_glm_log, main = "Residuos Modelo Gamma link log")
```
...
Al comparar estos residuos con las distancias de Cook, podemos identificar si los puntos considerados influyentes (según la distancia de Cook) son también outliers (según los residuos). Esto es importante porque un punto puede ser influyente sin ser un outlier (afecta significativamente el modelo pero no es necesariamente un valor atípico en términos de residuos), y un outlier puede no ser necesariamente influyente.

```{r}

# Calculamos los residuos del modelo
residuos <- residuals(stepwise_forward)

# Calculamos el percentil 90 para los residuos
percentil_90_residuos <- quantile(residuos, 0.90)

# Calculamos los residuos absolutos y su percentil 90
residuos_absolutos <- abs(residuos)
percentil_90_residuos_absolutos <- quantile(residuos_absolutos, 0.90)

# Creamos un dataframe para los puntos influyentes
comparacion <- data.frame(
  Indice = puntos_influyentes,
  DistanciaCooks = cooks_distance[puntos_influyentes]
)

# Calculamos el porcentaje de cada residuo absoluto en relación al percentil 90
comparacion$PorcentajeResiduo = sapply(residuos_absolutos[puntos_influyentes], function(x) {
  (ecdf(residuos_absolutos)(x)) * 100
})

# Imprimimos el dataframe con los porcentajes
print(comparacion)
```
Observamos que los punto influyente coincide tienen un porcentaje residual muy alto. Optamos por eliminarlos para mejorar el rendimiento y la interpretabilidad de nuestro modelo.
...
ggplot(resultados, aes(x = Particion)) +
  geom_line(aes(y = MAPE, group = 1, colour = "MAPE")) +
  labs(title = "Distribución de MAE y MAPE a través de las particiones",
       y = "Valor",
       colour = "Métrica") +
  theme_minimal()
```
...
```{r}
modelo_defensas <- glm(value~., data = def, family = Gamma(link = "log"))
predicciones <- predict(modelo_defensas, newdata = delanteros_numerico_reducido, type = "response")
error<-defensas_numérica_reducido$value - predicciones
defensas$valuepred<-predicciones
defensas$error<-error
defensas$error_porcentual<-(defensas$error / delanteros$value )*100
ecdf_func<-ecdf(delanteros$error_porcentual)
defensas$percentil<-ecdf_func(defensas$error_porcentual )*100
```
...
defensas<-defensas %>% relocate(percentil, .after = error_porcentual)
```

```{r}
sobre_valorados<-defensas %>%
                group_by(error) %>% 
                summarise(Player, value, valuepred, error,error_porcentual,percentil) %>%
                filter(percentil<90)%>% 
                slice_max(n=5, order_by = error_porcentual, with_ties = TRUE)
sobre_valorados
```

```{r}
# Gráfico BreakDown para los 5 jugadores más infravalorados.
infra_valorados<-defensas%>%
                group_by(error) %>% 
                summarise(Player, value, valuepred, error,error_porcentual,percentil) %>%
                filter(percentil>10)%>% 
                slice_min(n=5, order_by = error_porcentual, with_ties = TRUE)
infra_valorados
```

```{r}
# Gráfico Breakdown de los 5 jugadores Sobrevalorados
library(breakDown)
lista_sobre<-defensas%>%filter(Player %in% sobre_valorados$Player)
lista_br<-list()

for (i in 1:nrow(lista_infra)){
  br_plot<-plot(broken(modelo_defensas, lista_sobre[i,]))
  br_plot<-br_plot + ggtitle("Gráfico Breakdown - Sobrevalorado",lista_sobre$Player[i])
 print(br_plot)
}
...
```{r}
# Gráfico Breakdown de los 5 jugadores Infravalorados
library(breakDown)
lista_infra<-defensas%>%filter(Player %in% infra_valorados$Player)
lista_br<-list()

for (i in 1:nrow(lista_infra)){
  br_plot<-plot(broken(modelo_defensas, lista_infra[i,]))
  br_plot<-br_plot + ggtitle("Gráfico BreakDown - Infravalorado",lista_infra$Player[i])
 print(br_plot)
}
...
```{r}
write.csv(delanteros, file = "defensas", row.names = FALSE)
```


```{r}
defensas_numérica_reducido <- defensas_numérica_reducido[, !(names(defensas_numérica_reducido) %in% c(
 "Born",
  "Starts",
  "Min",
  "PKatt",
...
write.csv(defensas_numérica_reducido, file = "defensas_Rforest_dataset.csv", row.names = FALSE)
```



Repetimos el mismo procedimiento para el conjunto de datos de los jugadores delanteros. Primero eliminamos todas las variables que creemos que nos están dando información redudante. 
...
# Histograma
ggplot(delanteros, aes(x = value)) +
  geom_histogram(aes(y = ..density..), binwidth = 1, fill = "blue", alpha = 0.7) +
  geom_density(colour = "red", size = 1) +
  ggtitle("Histograma Delanteros")


# Q-Q Plot
qqnorm(delanteros$value)
qqline(delanteros$value, col = "red")

```

```{r}

delanteros_numerico <- delanteros[, sapply(delanteros, is.numeric)]

# identificamos y eliminamos las variables constantes
constant_vars <- sapply(delanteros_numerico, function(col) length(unique(col)) == 1)
delanteros_numerico <- delanteros_numerico[, !constant_vars]

cat("Variables eliminadas:", paste(names(delanteros)[constant_vars], collapse = ", "), "\n")

# calculamos la matriz de correlaciones tras eliminar las variables constantes
matriz_correlacion <- cor(delanteros_numerico, method = "pearson")
...
delanteros_numerico_reducido <- delanteros_numerico[, !(names(delanteros_numerico) %in% c(
  "Born",       
  "Min",       
  "MP",            
  "Starts",           
...
# Verificamos el resultado
print(delanteros_numerico_reducido)
```
...
#Shapiro test 
shapiro_result <- shapiro.test(delanteros_numerico_reducido$value)
print(shapiro_result)

#Modelo Gamma (link = log)
glm_del_modelo_log <- glm(value ~ ., data = delanteros_numerico_reducido, Gamma(link = "log"))

summary(glm_del_modelo_log)
```
El valor muy bajo de W y un valor p significativamente menor que 0.05 sugieren que los datos de delanteros_numerico_reducido$value no siguen una distribución normal. 
...
k <- 3
n <- nrow(delanteros_numerico_reducido)
indices <- sample(1:k, n, replace = TRUE)
folds <- split(delanteros_numerico_reducido, indices)
...
# Calculamos la mediana de MAE y MAPE
median_mae <- median(resultados$MAE)
median_mape <- median(resultados$MAPE)

# Imprimimos la mediana de MAE y MAPE
print(paste("Mediana MAE:", median_mae))
print(paste("Mediana MAPE:", median_mape))

# Graficamos la distribución de MAE y MAPE
ggplot(resultados, aes(x = Particion)) +
  geom_line(aes(y = MAPE, group = 1, colour = "MAPE")) +
  labs(title = "Distribución de MAPE a través de las particiones (Delanteros)",
       y = "Valor",
       colour = "Métrica") +
  theme_minimal()
```
...
```{r}
# Calculamos los residuos del modelo
residuos_glm_log <- residuals(modelo)
plot(modelo, main = "Residuos Modelo Gamma link log (Delanteros)")
```
Considerando la mediana, las predicciones del modelo se desvían de los valores reales en aproximadamente 30 unidades en la escala de la variable de respuesta (miliones). El MAPE de 257.80% sugiere que hay un error de predicción del 257.80% en relación con los valores reales. Los residuos parecen estar dispersos alrededor de la línea cero sin ningún patrón obvio, lo que  indica un buen ajuste con el modelo Gamma y link long.
...
#Modelo Gamma link = identity
glm_modelo_identity <- glm(value ~ ., 
                           data = delanteros_numerico_reducido, 
                           family = Gamma(link = "identity"))
...
# Imprimimos la media de MAE y MAPE
print(paste("Mediana MAE:", median_mae))
print(paste("Mediana MAPE:", median_mape))
...
# Ajuste modelo Gamma log sólo con variables estadísticamente significativas
for (i in 1:108) {
  nuevo_modelo_glm <- glm(value ~ ., data = delanteros_numerico_reducido, family = Gamma(link = "log"))
  coef_summary <- coef(summary(nuevo_modelo_glm))
  p_values <- coef_summary[-1, "Pr(>|t|)"]  # Excluye el intercepto
  max_p_value <- max(p_values)
  if (max_p_value > 0.05) {
    max_p_var <- names(which(p_values == max_p_value))
    delanteros_numerico_reducido <- delanteros_numerico_reducido[, !names(delanteros_numerico_reducido) %in% max_p_var]
  } else {
    break  # Detiene el bucle si todas las variables son significativas
  }
}
...
# Almacenamos los nombres de las variables del modelo final
variables_significativas <- names(coef(nuevo_modelo_glm))
...
for(i in 1:k) {
  ...
  # Asegurarse de usar solo las variables significativas en el modelo
  formula <- as.formula(paste("value ~", paste(variables_significativas[-1], collapse = " + ")))
  modelo <- glm(formula, data = conjunto_entrenamiento, family = Gamma(link = "log"))
  predicciones <- predict(modelo, newdata = conjunto_prueba, type = "response")
...
plot(fitted(stepwise_forward), residuals(stepwise_forward, type = "pearson"),
     xlab = "Valores Ajustados", ylab = "Residuos",
     main = "Residuos vs Valores Ajustados (Delanteros)")
abline(h = 0, col = "red")
```
Parece haber una dispersión aleatoria de residuos, lo que generalmente es una buena señal. No hay una evidencia clara de heteroscedasticidad (variabilidad de los residuos cambiando con los valores ajustados), aunque los puntos parecen dispersarse ligeramente más a medida que los valores ajustados aumentan. 

Hay algunos puntos que están más alejados de la línea central en comparación con la mayoría de los datos, especialmente uno notorio cerca del extremo derecho del gráfico. Esto puede indicar la presencia de valores atípicos que podrían influir en el modelo de manera desproporcionada.
...
#Dwtest
dw_test <- dwtest(stepwise_forward)
dw_test
```
Los puntos se adhieren bastante bien a la línea en la mayoría de las secciones, lo que sugiere que los datos son aproximadamente normales.Mientras tanto, la prueba de Durbin-Watson indica que hay autocorrelación positiva en los residuos del modelo de regresión. 
...
print(puntos_influyentes)
```
La mayoría de los puntos se concentran alrededor de la línea horizontal que indica residuos estándar cercanos a cero, lo cual es positivo. Sin embargo, hay puntos con residuos altos y apalancamiento significativo (como el punto 176), lo que sugiere que podrían ser valores atípicos o casos influyentes.
...
```{r}
delantero_sin_influyentes <- delanteros_numerico_reducido[-puntos_influyentes, ]
glm_modelo_sin_influyentes <- glm(value~., data = delantero_sin_influyentes, family = Gamma(link = "log"))

summary(glm_modelo_sin_influyentes)
```

```{r}
# Ajuste modelo  sólo con variables estadísticamente significativas a 0.01

for (i in 1:108) {
  glm_modelo_sin_influyentes <- glm(value ~ ., data = delantero_sin_influyentes, family = Gamma(link = "log"))
  coef_summary <- coef(summary(glm_modelo_sin_influyentes))
  p_values <- coef_summary[-1, "Pr(>|t|)"]  # Excluye el intercepto
  max_p_value <- max(p_values)
  if (max_p_value > 0.01) {  # Cambiamos el criterio de significancia a 0.01
    max_p_var <- names(which(p_values == max_p_value))
    delantero_sin_influyentes <- delantero_sin_influyentes[, !names(delantero_sin_influyentes) %in% max_p_var]
  } else {
    break  # Detiene el bucle si todas las variables son significativas según el nuevo criterio
  }
}
...
k <- 3
n <- nrow(delantero_sin_influyentes)
indices <- sample(1:k, n, replace = TRUE)
folds <- split(delantero_sin_influyentes, indices)
...
ggplot(resultados, aes(x = Particion)) +
  geom_line(aes(y = MAPE, group = 1, colour = "MAPE")) +
  labs(title = "Distribución de MAE y MAPE a través de las particiones (Delanteros)",
       y = "Valor",
       colour = "Métrica") +
  theme_minimal()
```
...
predicciones <- predict(glm_modelo_sin_influyentes, newdata = delanteros_numerico_reducido, type = "response")
error<-delanteros_numerico_reducido$value - predicciones
delanteros$valuepred<-predicciones
delanteros$error<-error
delanteros$error_porcentual<-(delanteros$error / delanteros$value )*100
ecdf_func<-ecdf(delanteros$error_porcentual)
delanteros$percentil<-ecdf_func(delanteros$error_porcentual )*100
```

```{r}
options(fig.width=400, fig.height=40)
ggplot(delanteros, aes(x = as.numeric(row.names(delanteros)))) +
  geom_line(aes(x=as.numeric(row.names(delanteros)),y = valuepred, colour = "Prediccion")) + 
  geom_line(aes(x=as.numeric(row.names(delanteros)),y = value, colour = "Real")) +
  labs(title = "Predicción vs Valor Real",
       y = "Valor",
       x = "Index",
       color='Legend') +
...
```{r}
delanteros<-delanteros %>% relocate(valuepred, .after = value)
delanteros<-delanteros %>% relocate(error, .after = valuepred)
delanteros<-delanteros %>% relocate(error_porcentual, .after = error)
delanteros<-delanteros %>% relocate(percentil, .after = error_porcentual)
```

```{r}
#Jugadores sobrevalorados
sobre_valorados<-delanteros %>%
                group_by(error) %>% 
                summarise(Player, value, valuepred, error,error_porcentual,percentil) %>%
                filter(percentil<90)%>% 
                slice_max(n=5, order_by = error_porcentual, with_ties = TRUE)
...
# Gráfico Breakdown de los 5 jugadores Sobrevalorados
library(breakDown)
lista_sobre<-delanteros%>%filter(Player %in% sobre_valorados$Player)
lista_br<-list()

for (i in 1:nrow(lista_infra)){
  br_plot<-plot(broken(glm_modelo_sin_influyentes, lista_sobre[i,]))
  br_plot<-br_plot + ggtitle("Gráfico Breakdown - Sobrevalorado",lista_sobre$Player[i])
 print(br_plot)
}
...
# Gráfico Breakdown de los 5 jugadores Infravalorados
library(breakDown)
lista_infra<-delanteros%>%filter(Player %in% infra_valorados$Player)
lista_br<-list()

for (i in 1:nrow(lista_infra)){
  br_plot<-plot(broken(glm_modelo_sin_influyentes, lista_infra[i,]))
  br_plot<-br_plot + ggtitle("Gráfico BreakDown - Infravalorado",lista_infra$Player[i])
 print(br_plot)
}
...
```{r}
MedianValue <- median(delanteros$value)
delanteros$percentil<-ifelse(delanteros$value>MedianValue,'Alto','Bajo')
delanteros$sobreinfra<-ifelse(delanteros$error_porcentual>0,'Sobrevalorado','Infravalorado')

sobrevalorados<-delanteros[delanteros$sobreinfra=='Sobrevalorado',]
infravalorados<-delanteros[delanteros$sobreinfra=='Infravalorado',]
...
```{r}
delanteros_numerico_reducido <- delanteros_numerico_reducido[, !(names(delanteros_numerico_reducido) %in% c(
 "Born",
  "Starts",
  "Min",
  "PKatt",
...
write.csv(delanteros_numerico_reducido, file = "delanteros_Rforest_dataset.csv", row.names = FALSE)
```
