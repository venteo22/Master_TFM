...
# Correlación de Pearson
text(paste("Correlación:", round(cor(sorare_defensa$normvalue, sorare_defensa$normmediana), 2)), x = 25, y = 95)
```
```{r}
# Creamos el gráfico
...
ggplot(centrocampistas, aes(x = value)) +
  geom_histogram(aes(y = ..density..), binwidth = 1, fill = "blue", alpha = 0.7) +
  geom_density(colour = "red", size = 1)

# Q-Q Plot
qqnorm(centrocampistas$value)
qqline(centrocampistas$value, col = "red")
...
Construimos tres modelos lineales según la categoría del jugador. Primero empezamos con los jugadores centrocampistas. 


```{r}
#Seleccionamos variables numéricas para Centrocampistas
centrocampistas_numérica <- centrocampistas[, sapply(centrocampistas, is.numeric)]
print(centrocampistas_numérica)
```
```{r}
# Utilizamos pearson para variables numéricas.

matriz_correlacion<- cor(centrocampistas_numérica, method = "pearson")
...
Eliminamos Born 
-----------------------
Starts : Partidos comenzados
Min : Minutos jugados
X90s :Minutos jugados divididos en 90

Eliminamos Stars y Min, todas relacionadas con el tiempo de juego
-----------------------

PKatt : Intentos de tiros de penal 
ShoPK : Tiros de penal realizados

Eliminamos Pkatt, ambas variables relacionadas con los tiros de penal
-----------------------
PasTotAtt : Intentos de pases
PasAtt : Pases intentados
PasTotCmp : Pases completados
PasTotDist : distancia de pases completados 
PasMedCmp : distancia de pases media distancia completados
PasLive : Pases con pelota viva
PasCmp : Pases completados
TouLive : Toques de balón (pases)
Touches : Número de veces que el jugador toca el balón (pases recibidos, etc.)

Eliminamos todas las variables excepto Touches, como se observa están todas relacionadas con los pases.
-----------------------
PasShoAtt :intento de pases corta distancia
passShoCmp: pases corta distancia completados

Eliminamos PasShoAtt, variables realcionadas con pases de corta distancia.
-----------------------
CkOut : Tiros de esquina superados 
CK :Tiros de esquina

Eliminamos CkOut, variables relacionadas con los tiros de esquina.
-----------------------
Tkl.Int :Números de jugadores bloqueados + intercepciones
Tkl: Números de jugadores bloqueados

Eliminamos Tkl.Int, variables relacionadas con los bloqueos a jugadores rivales.
-----------------------
BlkPass :Número de veces bloqueo de pases en el lugar
Blocks: Números de veces de bloqueo de balón en el lugar

Eliminamos Blkpas, variables relacionadas con el bloqueo estando en el lugar.
-----------------------
CarPrgDist :Total Distancia recorrida con balón controlado hacia la portería contraria
CarTotDist :Total de distancia de movimiento de balon controlado en cualquier dirección

Eliminamos CarPrgDist, variables relacionadas con distancia recorrida con balón controlado.
-----------------------
...
```{r}
centrocampistas_numérica_reducido <- centrocampistas_numérica[, !(names(centrocampistas_numérica) %in% c(
  "Born",
  "Starts",
  "Min",
  "PKatt",
...
#Shapiro test 
shapiro_result <- shapiro.test(centrocampistas_numérica_reducido$value)
print(shapiro_result)

#Modelo Gamma (link = log)
glm_modelo_log <- glm(value ~ ., data = centrocampistas_numérica_reducido, Gamma(link = "log"))
...
Goals: Significativa con un valor p < 0.05, sugiriendo una relación positiva.

PasMedCmp.: Significativa con un valor p < 0.05, sugiriendo una relación positiva.

TB : También significativa con un valor p < 0.002, mostrando una relación positiva.

CkIn : Significativa con un valor p < 0.05, sugiriendo una relación positiva.

Int : Significativa con un valor p < 0.05, sugiriendo una relación positiva.

Carries : También significativa con un valor p < 0.001, mostrando una relación negativa.

Rec : significativa con un valor p < 0.001, mostrando una relación positiva.

Fld : significativa con un valor p < 0.003, mostrando una relación negativa.
...
k <- 3
n <- nrow(centrocampistas_numérica_reducido)
indices <- sample(1:k, n, replace = TRUE)
folds <- split(centrocampistas_numérica_reducido, indices)
...
ggplot(resultados, aes(x = Particion)) +
  ...
  labs(title = "Distribución de MAPE a través de las particiones (Centrocampistas)",
       y = "Valor",
       colour = "Métrica") +
  theme_minimal()
```
...
plot(residuos_glm_log, main = "Residuos Modelo Gamma link log (Centrocampistas)")
```
Considerando la mediana, las predicciones del modelo se desvían de los valores reales en aproximadamente 24.75 unidades en la escala de la variable de respuesta (miliones). El MAPE de 251.61% sugiere que hay un error de predicción del 251.61% en relación con los valores reales. Los residuos parecen estar dispersos alrededor de la línea cero sin ningún patrón obvio, lo que  indica un buen ajuste con el modelo Gamma y link log.

Probamos el enlace identity para comprobar qué enlace se adapta mejor a nuestro modelo Gamma.

```{r}
##### arroja error modelo no adecuado para el tipo de variables....

#Modelo Gamma link = identity

#formula_gamma <- as.formula("value~.")
#glm_modelo_identity <- glm(formula_gamma,data = centrocampistas_numérica_reducido,family = Gamma(link = "identity"))
...
#                           data = centrocampistas_numérica_reducido, 
...
Modelo no converge con link inverse, debido a que la distribución de la variable predictora es asimetrica positiva.
...
for (i in c(1:numVars)){
  nuevo_modelo_glm <- glm(value ~ ., data = centrocampistas_numérica_reducido, Gamma(link = "log"))
  maxVar = max(coef(summary(nuevo_modelo_glm))[c(2:numVars), "Pr(>|t|)"])
  if (maxVar > 0.05){
    j = which(coef(summary(nuevo_modelo_glm))[c(2:numVars), "Pr(>|t|)"] == maxVar)
    centrocampistas_numérica_reducido = centrocampistas_numérica_reducido[, -(j+1)]
  }
  numVars = numVars - 1
}
...
```
```{r}
numVars = length(centrocampistas_numérica_reducido)
for (i in c(1:numVars)){
  nuevo_modelo_glm <- glm(value ~ ., data = centrocampistas_numérica_reducido, Gamma(link = "log"))
  maxVar = max(coef(summary(nuevo_modelo_glm))[c(2:numVars), "Pr(>|t|)"])
  if (maxVar > 0.01){
...
plot(fitted(stepwise_forward), residuals(stepwise_forward, type = "pearson"),
     ...
     main = "Residuos vs Valores Ajustados (Centrocampistas)")
abline(h = 0, col = "red")
```
...
El valor del estadístico Durbin-Watson es 1.7453 implica una auto correlación positiva en los residuos del modelo. Esto contradice la suposición de independencia de los residuos, una suposición clave en la regresión lineal.
...
lof_influyente <- centrocampistas$outliersestad[puntos_influyentes]
...
print(comparacion)
```
3 punto influyente coincide con los valores atípicos encontrados mediante el análisis LOF (observación 172,176 y 270). La falta de correspondencia podría indicar que los puntos influyentes no son necesariamente valores atípicos en el sentido típico, sino que tienen un impacto significativo en el ajuste del modelo. Estos puntos podrían ser extremos en términos de las variables predictoras, pero no necesariamente en la variable de respuesta. Optamos por eliminarlos para mejorar el rendimiento y la interpretabilidad de nuestro modelo.

```{r}
centrocampistas_sin_influyentes <- centrocampistas_numérica_reducido[-puntos_influyentes, ]
glm_modelo_sin_influyentes <- glm(value~., data = centrocampistas_sin_influyentes, family = Gamma(link = "log"))
...
k <- 3
n <- nrow(centrocampistas_sin_influyentes)
indices <- sample(1:k, n, replace = TRUE)
folds <- split(centrocampistas_sin_influyentes, indices)
...
ggplot(resultados, aes(x = Particion)) +
  geom_line(aes(y = MAPE, group = 1, colour = "MAPE")) +
  labs(title = "Distribución de MAE y MAPE a través de las particiones (Centrocampistas)",
       y = "Valor",
       colour = "Métrica") +
  theme_minimal()
```

Por lo general, las predicciones de nuestro modelo se alejan unas 8,4 unidades de los valores reales. De media, las predicciones del modelo se alejan un 68,6% del valor real. A pesar de que mediante los cambios que hemos ido implementando, estos valoes han bajado, este porcentaje de error es significativo indica que el modelo no funciona bien.  
...
```{r}
modelo_centrocampistas <- glm(value~., data = centrocampistas_numérica_reducido, family = Gamma(link = "log"))
#predicciones <- predict(modelo_centrocampistas, newdata = centrocampistas_numérica_reducido, type = "response")
summary(modelo_centrocampistas)
predicciones <- predict(modelo_centrocampistas, newdata = centrocampistas_numérica_reducido, type = "response")

error<-centrocampistas_numérica_reducido$value - predicciones
error_porcentual<-(centrocampistas_numérica_reducido$value - predicciones)/centrocampistas_numérica_reducido$value
centrocampistas_numérica_reducido$valuepred<-predicciones
centrocampistas_numérica_reducido$error<-error
centrocampistas$valuepred<-predicciones
centrocampistas$error<-error
centrocampistas$error_porcentual<-error_porcentual
```
```{r}
options(fig.width=400, fig.height=40)
ggplot(centrocampistas, aes(x = as.numeric(row.names(centrocampistas)))) +
  geom_line(aes(x=as.numeric(row.names(centrocampistas)),y = valuepred, colour = "Prediccion")) + 
  geom_line(aes(x=as.numeric(row.names(centrocampistas)),y = value, colour = "Real")) +
...
```{r}
centrocampistas<-centrocampistas %>% relocate(valuepred, .after = value)
centrocampistas<-centrocampistas %>% relocate(error, .after = valuepred)
centrocampistas<-centrocampistas %>% relocate(error_porcentual, .after = error)
#centrocampistas<-centrocampistas %>% relocate(percentil, .after = error_porcentual)
...
```{r}
MedianValue <- median(centrocampistas$value)
centrocampistas$percentil<-ifelse(centrocampistas$value>MedianValue,'Alto','Bajo')
centrocampistas$sobreinfra<-ifelse(centrocampistas$error_porcentual>0,'Sobrevalorado','Infravalorado')


sobrevalorados<-centrocampistas[centrocampistas$sobreinfra=='Sobrevalorado',]
infravalorados<-centrocampistas[centrocampistas$sobreinfra=='Infravalorado',]
...
```{r}
# Gráfico Breakdown  Sobrevalorados
explain_1 <- broken(modelo_centrocampistas, centrocampistas[centrocampistas$Player == "Andre Gomes", ])
explain_1
plot(explain_1) + ggtitle("Gráfico breakDown (Andre Gomes - Sobrevalorado)")

# Gráfico Breakdown Infravlorados
explain_1 <- broken(modelo_centrocampistas, centrocampistas[centrocampistas$Player == "Ilaix Moriba", ])
explain_1
plot(explain_1) + ggtitle("Gráfico breakDown (Ilaix Moriba - Infravalorado)")
...
```{r}
write.csv(centrocampistas, file = "centrocampistas.csv", row.names = FALSE)
```



Cambiar a un modelo Random Forest podría ser una buena estrategia si nos enfrentamos a una autocorrelación positiva en los residuos de su modelo lineal, sobre todo porque Random Forest puede capturar relaciones complejas y no lineales entre las características y la variable objetivo, así como manejar bien las variables multicolineales.
...
write.csv(centrocampistas_numérica_reducido, file = "Rforest_centrocampistas.csv", row.names = FALSE)
```
